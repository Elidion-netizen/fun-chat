...
import { filterMessages } from '@/helpers/filter-messages';
...
export function ChatPage(): React.JSX.Element {
  ...
  const { filteredMessages, unreadCounts } = React.useMemo(() => {
    return filterMessages({
      ...
      autoReadEnabled,
      talker,
      currentUserLogin: currentUserRef.current?.login,
      sendMessage,
      activateChatEvent,
    });
    ...
    <section className="h-screen">
      <header className="flex justify-between items-center px-5 py-2">
        ...
        <div className="flex align-middle">
          ...
          <button
            className="ml-3 py-2 px-4 bg-indigo-400 text-white font-bold w-full text-center rounded cursor-pointer hover:bg-indigo-500 transition-colors duration-400 ease-in-out"
            onClick={logout}
          >
            Logout
          </button>
      ...
      <footer className="flex justify-between items-center px-5 py-2">
        ...
        <a href="https://github.com/Elidion-netizen">Elidion-netizen</a>
        <p>2025</p>
      </footer>
    </section>
  );
